...
)
{
    ...
    TextInputDialog(
        ...
        onConfirm = {
            when (inputDialogTitle.value) {
                "Enter calorie amount" -> viewModel.onEvent(CreateFoodEvent.KcalChanged(inputTextKcal.value))
                "Enter carbohydrate amount" ->viewModel.onEvent(CreateFoodEvent.CarbsChanged(inputTextCarbs.value))
                "Enter protein amount" ->viewModel.onEvent(CreateFoodEvent.ProteinChanged(inputTextProtein.value))
                "Enter fat amount" ->viewModel.onEvent(CreateFoodEvent.FatChanged(inputTextFat.value))
                "Enter food name" ->viewModel.onEvent(CreateFoodEvent.FoodNameChanged(inputTextFoodName.value))
            }
            inputDialogState.value = false
        },
